...
class CloudWatchLogsAgentSetup:
    ...
    def load_local_config(self):
        ...
        if os.path.exists(fname):
            ...
            subprocess.call([AWSCLI_CMD, 'configure', 'set', 'aws_access_key_id', os.environ['AWS_ACCESS_KEY_ID']], env=DEFAULT_ENV)
            subprocess.call([AWSCLI_CMD, 'configure', 'set', 'aws_secret_access_key', os.environ['AWS_SECRET_ACCESS_KEY']], env=DEFAULT_ENV)
            subprocess.call([AWSCLI_CMD, 'configure', 'set', 'default.output', os.environ['AWS_OUTPUT']], env=DEFAULT_ENV)
            subprocess.call([AWSCLI_CMD, 'configure', 'set', 'default.region', os.environ['AWS_REGION']], env=DEFAULT_ENV)
        elif self.region:
